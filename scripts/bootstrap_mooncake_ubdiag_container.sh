...
# Mooncake Store + vendored UbDiag benchmark/RPM verification.
set -Eeuo pipefail
...
[ "${USE_UB:-}" = "ON" ] || [ "${USE_UB:-}" = "OFF" ] || \
    fatal "离线仓 USE_UB 必须是 ON 或 OFF: ${USE_UB:-missing}"
...
echo "  transport: $([ "$USE_UB" = "ON" ] && echo UB || echo TCP)"
[ "$USE_UB" = "OFF" ] || echo "  device: $DEVICE_NAME"
echo "============================================================"
...
mkdir -p "$OFFLINE_SOURCE_DIR/ubdiag" "$OFFLINE_GO_CACHE_DIR/build"
tar --no-same-owner -C "$OFFLINE_SOURCE_DIR/ubdiag" \
    -xzf "$OFFLINE_BUNDLE_DIR/ubdiag-source.tar.gz"
if [ "$USE_UB" = "ON" ]; then
    mkdir -p "$OFFLINE_SOURCE_DIR/urma"
...
cat >"$ENV_FILE" <<EOF
unset URMA_LIBRARY FETCHCONTENT_SOURCE_DIR_URMA DEVICE_NAME
...
export USE_UB="$USE_UB"
export PROTOCOL="$([ "$USE_UB" = "ON" ] && echo ub || echo tcp)"
...
if [ "$USE_UB" = "ON" ]; then
    required_headers+=(/usr/include/ub/umdk/urma/urma_api.h)
fi
...
echo "[3/5] 检查动态库..."
required_libraries=(
    libibverbs.so libnuma.so libyaml-cpp.so libgflags.so
    libglog.so libjsoncpp.so libcurl.so liburing.so libxxhash.so
    libzstd.so
)
if [ "$USE_UB" = "ON" ]; then
    required_libraries+=(liburma.so)
fi
...
URMA_LOG_LEVEL=debug /tmp/mooncake_urma_init_probe
    ...
    echo "  SKIP: USE_UB=OFF，不需要 URMA runtime/provider"
fi
...
[ "$(ulimit -l)" = "unlimited" ] || fatal "memlock 不是 unlimited: $(ulimit -l)"
    ...
    echo "  SKIP: TCP 验证不需要 UB 设备"
fi

echo ""
echo "============================================================"
echo "  PASS: Mooncake UbDiag 容器依赖和环境变量均已就绪"
